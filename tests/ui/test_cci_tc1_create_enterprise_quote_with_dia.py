"""TC1 — Create Enterprise Quote with DIA (CCI Sandbox).

Test metadata (display name, tags, objective) lives in the class
attributes below; the dashboard parser reads them via AST.

Data:      tests/ui/data/tc1_create_enterprise_quote_with_dia.json

Run:       cci test tests/ui/test_cci_tc1_create_enterprise_quote_with_dia.py
Headless:  cci test tests/ui/test_cci_tc1_create_enterprise_quote_with_dia.py --headless
"""
...
_DATA_FILE = Path(__file__).parent / "data" / "tc1_create_enterprise_quote_with_dia.json"
...
PRODUCT_SEARCH = PRODUCT["search_term"]           # "Dedicated Internet Access (DIA)"
PRODUCT_DISPLAY = PRODUCT["display_name"]          # "Dedicated Internet Access (DIA)"
PRODUCT_BANDWIDTH = PRODUCT["bandwidth"]           # "100 Mbps"
EXPECTED_SUMMARY = PRODUCT["expected_summary_products"]  # ["DIA", "Router", "Connection"]

ROUTER = DATA.get("router", {})
ROUTER_PROVIDED_BY = ROUTER.get("provided_by", "Fidium Provided")
ROUTER_PROVIDED_BY_LABEL = ROUTER.get("provided_by_field_label", "Provided By")

DEFAULT_TIMEOUT = DATA.get("timeout_ms", 60000)


class TestCreateEnterpriseQuoteWithDIA:
    """TC1 - Create Enterprise Quote with DIA"""

    # Class-level metadata read by the dashboard parser (no YAML needed).
    # Placeholders like {product.bandwidth} are resolved against the
    # matching JSON data file (tests/ui/data/tc1_*.json) at display time.
    TAGS = ["account", "business", "opportunity", "quote",
            "enterprise", "dia", "product", "smoke"]
    OBJECTIVE = (
        "End-to-end flow that creates a Business Account, Opportunity, "
        "and Enterprise Quote with a Dedicated Internet Access (DIA) "
        "product at {product.bandwidth}, then verifies the Summary tab "
        "(DIA + Router + Connection) and the Quote page's Approval "
        "Journey section."
    )
    ...
    def _wait_for_config_update_complete(self, label: str = "", timeout_ms: int = 45000):
        ...
        After changing a field (Router Provided By, DIA Bandwidth, etc.) the UI
        shows a blue toast like 'Updating Router' → then a green toast
        'Updated Router', and spinners may appear while re-rendering.
        ...
        # The toast text is like "Updating Router" or "Updating Dedicated Internet Access (DIA)"
    ...
    def test_create_enterprise_quote_with_dia(self):
        ...
        try:
            ...
            for spinner_sel in (
                ...
                try:
                    page.wait_for_selector(spinner_sel, state="hidden", timeout=45000)
                except Exception:
                    continue

            # 2. Location tab is the definitive "quote page loaded" signal
            loc_tab_ok = False
            deadline = page.evaluate("Date.now()") + 180000
            while page.evaluate("Date.now()") < deadline:
                try:
            ...
            if not loc_tab_ok:
                raise Exception(
                    "Enterprise Quote page did not render within 180s "
            ...
            for spinner_sel in (".slds-spinner", "lightning-spinner"):
                try:
                    page.wait_for_selector(spinner_sel, state="hidden", timeout=15000)
                except Exception:
                    continue
        ...
        # ===== STEP 18: Search for DIA and Add to Cart =====
        ...
        try:
            ...
            # ── Verify DIA appears in the results, then click its Add to Cart ──
            # First confirm the product name is visible in results
            dia_visible = False
            try:
                dia_text = page.get_by_text(
                    re.compile(re.escape("Dedicated Internet Access"), re.I)
                )
                if dia_text.count() > 0 and dia_text.first.is_visible(timeout=5000):
                    dia_visible = True
            except Exception:
                pass
            if not dia_visible:
                # Try shorter match
                try:
                    dia_text = page.get_by_text(re.compile(r"DIA", re.I))
                    if dia_text.count() > 0 and dia_text.first.is_visible(timeout=3000):
                        dia_visible = True
                except Exception:
                    pass
            ...
            # ── Click "Add to Cart" for the DIA product specifically ──
            # Strategy: find a product card/row that contains "Dedicated Internet
            # Access" or "DIA" text, then click "Add to Cart" within that container.
            added = False

            # Strategy 1: Find the container holding DIA text, then its Add to Cart
            for container_sel in (
                ...
                try:
                    containers = page.locator(container_sel).filter(
                        has_text=re.compile(r"Dedicated\s+Internet\s+Access|DIA", re.I)
                    )
            ...
            # DIA product text first (some UIs select on click), then Add to Cart
            if not added:
                try:
                    dia_el = page.get_by_text(
                        re.compile(re.escape("Dedicated Internet Access"), re.I)
                    ).first
                    dia_el.scroll_into_view_if_needed()
                    dia_el.click()
                    page.wait_for_timeout(1500)
            ...
            # and DIA is visible, the single Add to Cart button must be for DIA
            if not added:
                try:
                    atc_buttons = page.get_by_role(
                        "button", name=re.compile(r"Add\s*to\s*Cart", re.I)
                    )
                    if atc_buttons.count() == 1 and dia_visible:
        ...
        # ===== STEP 20: Update Router "Provided By" to Fidium Provided =====
        tracker.start_step(20, f"Set Router '{ROUTER_PROVIDED_BY_LABEL}' to '{ROUTER_PROVIDED_BY}'",
                           f"Update Router Provided By field (skip if already '{ROUTER_PROVIDED_BY}')")
        try:
            page.wait_for_timeout(2000)
            sf.screenshot("20a_config_page_before_router")

            # The configuration page shows multiple product sections.
            # We need to find the "Router" section and set "Provided By".
            # Strategy: find a section/card/accordion that contains "Router"
            # text, then within it find "Provided By" combobox/dropdown.

            router_provided_set = False

            # First check if "Provided By" is already set to the target value
            already_set = False
            try:
                # Look for text like "Fidium Provided" near "Provided By"
                provided_text = page.get_by_text(
                    re.compile(re.escape(ROUTER_PROVIDED_BY), re.I)
                )
                if provided_text.count() > 0 and provided_text.first.is_visible(timeout=3000):
                    already_set = True
            except Exception:
                pass
            ...
            if already_set:
                tracker.add_assertion(
                    f"Router '{ROUTER_PROVIDED_BY_LABEL}' already set to '{ROUTER_PROVIDED_BY}'", True
                )
                router_provided_set = True
            else:
                # Need to find and set the "Provided By" dropdown
                # Strategy 1: Find combobox/dropdown labeled "Provided By"
                for strat in (
                    lambda: page.get_by_role("combobox", name=re.compile(
                        rf"{re.escape(ROUTER_PROVIDED_BY_LABEL)}", re.I)),
                    lambda: page.get_by_label(re.compile(
                        rf"{re.escape(ROUTER_PROVIDED_BY_LABEL)}", re.I)),
                ):
                    try:
                        loc = strat()
                        if loc.count() > 0 and loc.first.is_visible():
                            ...
                            for opt_strat in (
                                lambda: page.get_by_role("option", name=re.compile(
                                    re.escape(ROUTER_PROVIDED_BY), re.I)),
                                lambda: page.get_by_text(re.compile(
                                    rf"^\s*{re.escape(ROUTER_PROVIDED_BY)}\s*$", re.I)),
                            ):
                                try:
                                    opt = opt_strat()
                                    ...
                                    continue
                            if router_provided_set:
                                break
                    except Exception:
                        continue

                # Strategy 2: Try native <select> near "Provided By" label
                if not router_provided_set:
                    try:
                        native = page.locator(
                            "xpath=//label[contains(normalize-space(.),'Provided By')]"
                            "/following::select[1]"
                        )
                        if native.count() > 0:
                            try:
                                native.first.select_option(label=ROUTER_PROVIDED_BY)
                                router_provided_set = True
                            except Exception:
                                try:
                                    native.first.select_option(
                                        value=ROUTER_PROVIDED_BY)
                                    router_provided_set = True
                                except Exception:
                                    pass
                    except Exception:
                ...
                # Strategy 3: Click a button/anchor trigger near "Provided By"
                if not router_provided_set:
                    try:
                        trigger = page.locator(
                            "xpath=//label[contains(normalize-space(.),'Provided By')]"
                ...
                if not router_provided_set:
                    raise Exception(
                        f"Could not set Router '{ROUTER_PROVIDED_BY_LABEL}' to "
                        f"'{ROUTER_PROVIDED_BY}'"
                    )
                tracker.add_assertion(
                    f"Router '{ROUTER_PROVIDED_BY_LABEL}' set to '{ROUTER_PROVIDED_BY}'", True
                )

            # Wait for "Updating Router" toast to disappear, spinners to clear,
            # and page to fully re-render before proceeding
            self._wait_for_config_update_complete("Router")
            tracker.pass_step(sf.screenshot("20b_router_provided_by_set"))
        except Exception as e:
            tracker.add_assertion(f"Router '{ROUTER_PROVIDED_BY_LABEL}' update", False)
            tracker.fail_step(str(e), sf.screenshot("20_router_FAILED"))
            pytest.fail(f"Step 20 - Router Provided By: {e}")

        # ===== STEP 21: Set DIA Bandwidth =====
        tracker.start_step(21, f"Set DIA Bandwidth to {PRODUCT_BANDWIDTH}",
                           "Configure the bandwidth dropdown on the product configuration page")
        try:
            page.wait_for_timeout(2000)
            sf.screenshot("21a_config_page_loaded")

            # Find bandwidth field — could be a combobox, dropdown, or input
            bw_set = False
            bw_value = PRODUCT_BANDWIDTH.split()[0] if " " in PRODUCT_BANDWIDTH else PRODUCT_BANDWIDTH

            # Strategy 1: Combobox / dropdown labeled "Bandwidth"
            for strat in (
                lambda: page.get_by_role("combobox", name=re.compile(r"bandwidth", re.I)),
                lambda: page.get_by_label(re.compile(r"bandwidth", re.I)),
            ):
                try:
                    loc = strat()
                    if loc.count() > 0 and loc.first.is_visible():
                        ...
                        page.wait_for_timeout(1000)
                        for opt_strat in (
                            lambda: page.get_by_role("option", name=re.compile(rf"{re.escape(bw_value)}", re.I)),
                            lambda: page.get_by_text(re.compile(rf"^\s*{re.escape(PRODUCT_BANDWIDTH)}\s*$", re.I)),
                            lambda: page.get_by_text(re.compile(rf"^\s*{re.escape(bw_value)}\s*$", re.I)),
                        ):
                            try:
                                opt = opt_strat()
                                if opt.count() > 0 and opt.first.is_visible():
            ...
            if not bw_set:
                for strat in (
                    lambda: page.get_by_label(re.compile(r"bandwidth", re.I)),
                    lambda: page.get_by_placeholder(re.compile(r"bandwidth", re.I)),
                ):
                    try:
                        loc = strat()
                        if loc.count() > 0 and loc.first.is_visible() and loc.first.is_editable():
                            loc.first.fill(bw_value)
                            bw_set = True
                            break
                    except Exception:
            ...
            if not bw_set:
                raise Exception(f"Could not set bandwidth to {PRODUCT_BANDWIDTH}")

            # Wait for "Updating Dedicated Internet Access (DIA)" toast to disappear,
            # spinners to clear, and page to fully re-render
            self._wait_for_config_update_complete("DIA")

            tracker.add_assertion(f"Bandwidth set to {PRODUCT_BANDWIDTH}", True)
            tracker.pass_step(sf.screenshot("21b_bandwidth_set"))
        except Exception as e:
            tracker.add_assertion(f"Bandwidth to {PRODUCT_BANDWIDTH}", False)
            tracker.fail_step(str(e), sf.screenshot("21_bandwidth_FAILED"))
            pytest.fail(f"Step 21 - DIA Bandwidth: {e}")

        # ===== STEP 22: Add product to location — return to Enterprise Quote =====
        tracker.start_step(22, "Add product to location — return to Enterprise Quote",
        ...
        try:
            ...
            sf.screenshot("22a_page_ready_before_click")
            ...
            if not clicked:
                sf.screenshot("22_btn_not_found")
                raise Exception(
            ...
            tracker.pass_step(sf.screenshot("22_back_on_eq"))
        except Exception as e:
            tracker.add_assertion("Add product to location", False)
            tracker.fail_step(str(e), sf.screenshot("22_add_to_location_FAILED"))
            pytest.fail(f"Step 22 - Add to Location: {e}")

        # ===== STEP 23: Verify Summary tab shows expected products =====
        tracker.start_step(23, "Verify Summary tab shows expected products",
        ...
        try:
            ...
            sf.screenshot("23a_eq_page_ready")
            ...
            if not summary_clicked:
                sf.screenshot("23_summary_tab_not_found")
            ...
            # The expected products are: "Dedicated Internet Access (DIA)",
            # "Connection", "Router".
            ...
            while page.evaluate("Date.now()") < _poll_deadline:
                for pname in ("Dedicated Internet Access", "Router", "Connection"):
                    try:
            ...
            sf.screenshot("23b_summary_tab_loaded")
            ...
            tracker.pass_step(sf.screenshot("23c_summary_verified"))
        except Exception as e:
            tracker.add_assertion("Summary products verified", False)
            tracker.fail_step(str(e), sf.screenshot("23_summary_FAILED"))
            pytest.fail(f"Step 23 - Summary Verification: {e}")

        # ===== STEP 24: Go to Quote — Verify Approval Journey =====
        tracker.start_step(24, "Go to Quote — Verify Approval Journey",
        ...
        try:
            ...
            if not clicked:
                sf.screenshot("24_go_to_quote_btn_not_found")
            ...
            sf.screenshot("24a_navigating_to_quote")
            ...
            sf.screenshot("24b_quote_page_loaded")
            ...
            # Parse it, then attach a clickable link BOTH to this step (24)
            ...
            try:
                ...
                if m:
                    ...
                    # Also attach to current step (24) for convenience
                    tracker.add_record(
                        "Quote", QUOTE_NAME,
                        record_id=self._quote_id, object_type=quote_object,
                    )
            ...
            if not aj_visible:
                sf.screenshot("24_approval_journey_not_found")
                raise Exception(
            ...
            tracker.pass_step(sf.screenshot("24c_approval_journey_verified"))
        except Exception as e:
            tracker.add_assertion("Quote page — Approval Journey", False)
            tracker.fail_step(str(e), sf.screenshot("24_quote_FAILED"))
            pytest.fail(f"Step 24 - Go to Quote: {e}")
