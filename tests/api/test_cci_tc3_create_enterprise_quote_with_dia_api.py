"""TC3 — Create Enterprise Quote with DIA (API-driven twin of TC1).

Test metadata (display name, tags, objective) lives in the class
attributes below; the dashboard parser reads them via AST.

Data:      tests/api/data/tc3_create_enterprise_quote_with_dia_api.json

Run:       cci test tests/api/test_cci_tc3_create_enterprise_quote_with_dia_api.py
...
BUILD STATUS:
  ...
  Phase 3 — Working-Cart flow (replicates the UI):
             ...
             2. Resolve Product2 + PricebookEntry ids.
             3. Spawn a transient Working Cart via IP
                ``create_WorkingCart``.
             4. Add DIA + Router to the WC via Vlocity CPQ v2 REST
                (POST /v2/cpq/carts/<workingCartId>/items).
             5. Configure attributes on WC items via CPQ v2 PUT.
...
_DATA_FILE = Path(__file__).parent / "data" / "tc3_create_enterprise_quote_with_dia_api.json"
...
def _collect_messages(body) -> list[dict]:
    ...
    response body. Used by Steps 14/15/17/18 to surface nested errors
    (Vlocity returns HTTP 200 with ``messages:[{severity:"ERROR"}]`` on
    some failures — silent 200-with-errors is the #1 source of
...
OPP_NAME = f"{OPP['name_prefix']}{TIMESTAMP}"   # mirrors TC1 UI-captured name
OPP_STAGE = OPP["stage"]
...
QUOTE = DATA["quote"]
QUOTE_NAME = f"{QUOTE['quote_name_prefix']}{ACCOUNT_NAME}"  # "CCIAUTO_Quote_CCIAUTO_Biz_<ts>"
QUOTE_RT_DEV_NAME = QUOTE.get("record_type_developer_name", "Enterprise_Quote")
...
QUOTE_SERVICE_TERM_LABEL = f"{QUOTE_SERVICE_TERM_MONTHS} Months"   # matches the UI picklist label

# Phase 3 — line items
PRODUCT = DATA["product"]
ROUTER = DATA["router"]
DIA_DISPLAY = PRODUCT.get("display_name", "Dedicated Internet Access (DIA)")
DIA_PRODUCT_CODE = PRODUCT.get("product_code", "CCI_COMM_DIA_UP_TO_2GBPS")
DIA_BANDWIDTH = PRODUCT.get("bandwidth", "200 Mbps")
DIA_BANDWIDTH_ATTR = PRODUCT.get("bandwidth_attr_code", "ATTR_BANDWIDTH")
ROUTER_DISPLAY = ROUTER.get("display_name", "Router")
ROUTER_PRODUCT_CODE = ROUTER.get("product_code", "CCI_COMM_EDGE_SBC")
ROUTER_PROVIDED_BY = ROUTER.get("provided_by", "Fidium Provided")
ROUTER_PROVIDED_BY_ATTR = ROUTER.get("provided_by_attr_code", "ATTR_PROVIDED_BY")
...
class TestCreateEnterpriseQuoteWithDIAApi:
    """TC3 - Create Enterprise Quote with DIA (API)"""

    # Class-level metadata read by the dashboard parser (no YAML needed).
    # Placeholders are resolved against tests/api/data/tc3_*.json.
    TAGS = ["api", "account", "business", "opportunity", "quote",
            "enterprise", "dia", "product", "quote_member", "working_cart"]
    OBJECTIVE = (
        "API-driven twin of TC1. Creates a Business Account, Opportunity, "
        "and Enterprise Quote with a Dedicated Internet Access (DIA) "
        "product at {product.bandwidth} using Salesforce REST + Vlocity "
        "CPQ APIs — no UI clicks. Mirrors the UI's Working-Cart flow: "
        "create the QuoteMember (location) on the Enterprise Quote, "
        "spawn a transient Working Cart, add line items there, then "
        "call AddQMQGToWC_CopyToEQ to copy them onto the EQ (attached "
        "to the QuoteMember) and dispose the WC. Same CCIAUTO marker "
        "and cleanup rules as TC1/TC2."
    )
    ...
    def setup(self, api_tracker, sf_api):
        ...
        self.tracker.extra_data = {
            ...
            "DIA Product": f"{DIA_DISPLAY} ({DIA_PRODUCT_CODE})",
            "DIA Bandwidth": DIA_BANDWIDTH,
            "Router": f"{ROUTER_DISPLAY} ({ROUTER_PRODUCT_CODE}) — {ROUTER_PROVIDED_BY}",
        ...
        # Phase 3 state
        self.dia_product_id: str | None = None
        self.dia_pbe_id: str | None = None
        self.router_product_id: str | None = None
        self.router_pbe_id: str | None = None
        self.dia_line_item_id: str | None = None
        self.router_line_item_id: str | None = None
    ...
    def test_create_enterprise_quote_with_dia_via_api(self):
        """TC3 — create Business Account, Opportunity, and Enterprise Quote
        (with DIA line items + finalization, once Phases 3–4 are wired) via
        Salesforce REST / Vlocity CPQ APIs. API-driven twin of TC1."""
        t = self.tracker
        sf_api = self.sf_api
        ...
        # Use describe() (not SOQL on RecordType) — the describe payload's
        # recordTypeInfos entries carry an `available` flag that reflects the
        # *current user's* profile/permission-set access. SOQL would happily
        # return a record type the API user can't actually assign, and the
        # subsequent Account create would 400 with INVALID_CROSS_REFERENCE_KEY.
        ...
        try:
            ...
            if not rt_id:
                raise RuntimeError(
                    ...
                    f"Alternatively, update 'record_type' in tc3_*.json to one of the available names."
                )
        ...
        # Note on addresses: this org has State & Country Picklists enabled,
        # so the REST API rejects free-form values on BillingCountry /
        # BillingState (FIELD_INTEGRITY_EXCEPTION). We send the ISO codes via
        # BillingCountryCode / BillingStateCode when available in the JSON —
        # Salesforce derives the display label automatically. If the JSON
        # only supplies the plain names, we fall back to BillingCountry /
        # BillingState (works on orgs without picklists enabled).
        ...
        # The user-facing "Create Opportunity - Business" button on Account
        # is a *standard Lightning Quick Action* (Account.New_Opportunity_Business) —
        # verified against the TC1 UI capture (aura saveQuickActionRecords
        # payload). It is NOT an OmniScript or Integration Procedure; it
        # simply DMLs Opportunity with RecordTypeId set to the Business RT.
        # Standard REST POST /sobjects/Opportunity with the same RecordTypeId
        # produces an identical record (triggers + validations fire the same
        # way). The historical bug here was a silent fallback when
        # pick_record_type returned None — the Opportunity got created with
        # the default (non-Business) RT, which downstream steps (Quote,
        # product configuration, order conversion) quietly misbehaved on.
        # We now FAIL HARD if the requested RT can't be resolved.
        ...
        try:
            ...
            if opp_rt_dev_name:
                # pick_record_type matches against both developerName and
                # name (space/underscore tolerant), so a JSON value of
                # "Business" resolves to Opportunity's "Business_Opportunity"
                # or "Business Opportunity" RT automatically.
                ...
                if not opp_rt_id:
                    raise RuntimeError(
                        ...
                        f"tc3_*.json to one of the available names."
                    )
        ...
        # Now includes a RecordType check — a previous bug let Step 5 create
        # an Opportunity with the default (non-Business) RT when the
        # describe-based RT lookup didn't match. The lookup is fixed, but
        # the verification stays as a tripwire against future regressions.
        ...
        try:
            ...
            if opp_rt_wanted:
                ...
                # Same tolerance as pick_record_type — a JSON value of
                # "Business" is satisfied by DeveloperName "Business" OR
                # "Business_Opportunity", and by Name "Business" OR
        ...
        # Same describe-based RT lookup pattern as Step 2 — honours the
        # running user's profile access.
        #
        # Namespace note: the CCI sandbox may host *either* Vlocity-managed-
        # package CPQ (``vlocity_cmt__*``) *or* Core-Omnistudio-on-Lightning
        # (unprefixed), and the simple _discover_namespace probe in
        # SFApiClient doesn't reliably distinguish "core omnistudio" from
        # "vlocity + something else". We therefore pick the Price List
        # object and the Quote custom fields *dynamically* via describe, so
        # the test works on either kind of org without a code change.
        t.start_step(
            7,
        ...
        try:
            ...
            # We look for the actual API names of the four Vlocity-style
            # fields we need; pick_field checks the namespace prefix for us.
            ...
            # Order: current namespace (from probe), then the two common
            # alternatives, then fully-unprefixed for core-Omnistudio.
            ns = sf_api.namespace
            pl_candidates = list(
                dict.fromkeys(
                    [
            ...
            if not pl_rows:
                raise RuntimeError(
                    ...
                    f"Verify 'quote.price_list_name' in tc3_*.json matches an existing "
        ...
        # This is the REST equivalent of the UI's
        #   inner::CpqAppHandler.createCart
        # Aura call captured during TC1. The Aura payload's ``inputFields``
        # array maps 1-to-1 onto fields we POST to /sobjects/Quote. Core
        # standard fields (Name, OpportunityId, RecordTypeId) sit alongside
        # CPQ fields (PriceListId, DefaultBillingAccountId, DefaultServiceAccountId)
        # whose API names were resolved by describe in Step 7b.
        ...
        try:
            ...
            # CCI-specific extended fields — the managed-package prefix
            # varies too. Probe Quote for matching suffixes; only include
            # fields that actually exist so we don't 400 on a phantom field.
            ...
            # Drop any optional fields that are empty — avoids sending
            # "" where Salesforce would prefer the field simply absent.
        ...
        # Replicates the UI's typeahead-address Save flow. Captured from
        # TC1 (Aura → ComponentController.handleData → vlocity_cmt IP
        # ESM_saveTypeaheadDetails). The IP upserts a
        # vlocity_cmt__QuoteMember__c with the passed location fields
        # (Google-geocoded address + lat/long + placeId) and returns the
        # new QuoteMember Id (a5j...). This Id is carried through Step
        # 15 (Copy-to-EQ) to link the line items to the location.
        #
        # We fail hard if the IP doesn't return a QuoteMember Id — without
        # one, Step 15 would copy items onto the EQ as unlocated orphans
        # (same bug the user flagged in TC1-vs-API parity review).
        t.start_step(
            10,
        ...
        try:
            if not LOCATION_MEMBER:
                raise RuntimeError(
                    "No 'location.member' block found in tc3_*.json. "
        ...
        # ── Step 11: Resolve DIA + Router Product2 + PricebookEntry ──────
        # Both product codes come from the TC1 Aura capture
        # (tc1_critical_aura_samples.json). We need Product2 (for identity)
        # and PricebookEntry (for the Vlocity CPQ v2 POST /items call,
        # which wants itemId = PricebookEntryId). The PricebookEntry must
        # also belong to the Pricebook2 attached to the Quote's Price List.
        t.start_step(
            11,
            f"Resolve Product2 + PricebookEntry for DIA ({DIA_PRODUCT_CODE}) "
                f"and Router ({ROUTER_PRODUCT_CODE})",
        )
        try:
            # Look up the Pricebook2Id linked to the Quote's Price List.
            # In Vlocity CMT, the Price List record has a custom field
            # (typically vlocity_cmt__Pricebook2Id__c) pointing at the
            # standard Pricebook2. Resolve it dynamically.
            ...
            if not pricebook_id:
                # Fallback: the standard Pricebook (IsStandard=true) is a
                # reasonable default for lookups; the CPQ call still uses
                # the Price List itself for pricing.
                sp = sf_api.soql(
            ...
            # Resolve Product2 + PricebookEntry for DIA
            dia_rows = sf_api.soql(
                f"SELECT Id, Name, ProductCode FROM Product2 "
                f"WHERE ProductCode='{DIA_PRODUCT_CODE}' AND IsActive=true LIMIT 1",
                name=f"SOQL: find DIA Product2 ({DIA_PRODUCT_CODE})",
            )
            if not dia_rows:
                raise RuntimeError(
                    f"Product2 with ProductCode '{DIA_PRODUCT_CODE}' not found."
                )
            self.dia_product_id = dia_rows[0]["Id"]
            dia_pbe_rows = sf_api.soql(
                f"SELECT Id, UnitPrice FROM PricebookEntry "
                f"WHERE Product2Id='{self.dia_product_id}' "
                f"AND Pricebook2Id='{pricebook_id}' AND IsActive=true LIMIT 1",
                name="SOQL: find DIA PricebookEntry",
            )
            if not dia_pbe_rows:
                raise RuntimeError(
                    f"No active PricebookEntry for DIA Product2 {self.dia_product_id} "
                    f"in Pricebook {pricebook_id}."
                )
            self.dia_pbe_id = dia_pbe_rows[0]["Id"]
            t.add_assertion(
                f"DIA resolved: Product2={self.dia_product_id}, PBE={self.dia_pbe_id}",
                True,
            )

            # Resolve Product2 + PricebookEntry for Router
            r_rows = sf_api.soql(
                f"SELECT Id, Name, ProductCode FROM Product2 "
                f"WHERE ProductCode='{ROUTER_PRODUCT_CODE}' AND IsActive=true LIMIT 1",
                name=f"SOQL: find Router Product2 ({ROUTER_PRODUCT_CODE})",
            )
            if not r_rows:
                raise RuntimeError(
                    f"Product2 with ProductCode '{ROUTER_PRODUCT_CODE}' not found."
                )
            self.router_product_id = r_rows[0]["Id"]
            r_pbe_rows = sf_api.soql(
                f"SELECT Id, UnitPrice FROM PricebookEntry "
                f"WHERE Product2Id='{self.router_product_id}' "
                f"AND Pricebook2Id='{pricebook_id}' AND IsActive=true LIMIT 1",
                name="SOQL: find Router PricebookEntry",
            )
            if not r_pbe_rows:
                raise RuntimeError(
                    f"No active PricebookEntry for Router Product2 "
                    f"{self.router_product_id} in Pricebook {pricebook_id}."
                )
            self.router_pbe_id = r_pbe_rows[0]["Id"]
            t.add_assertion(
                f"Router resolved: Product2={self.router_product_id}, PBE={self.router_pbe_id}",
                True,
            )

            # Ensure the Quote uses this Pricebook2 (required for QuoteLineItem
            # create; the CPQ v2 REST may set it too, but being explicit is safer).
            try:
                sf_api.update(
                    "Quote",
                    self.quote_id,
                ...
                # Non-fatal — some orgs lock Pricebook after Quote create.
        ...
        # Spawns a throwaway WC (distinct from the EQ) via IP
        # ``create_WorkingCart``. Items are added to THIS cart, not the
        # EQ, so Step 15's ``AddQMQGToWC_CopyToEQ`` has something to
        # dispose that isn't the EQ itself. The IP returns a
        # ``WorkingCartId`` (a different Quote record) that we carry
        # through Steps 13-14.
        t.start_step(
            12,
        ...
        # ── Step 13: Add DIA + Router to the Working Cart via CPQ v2 ─────
        # Call POST /services/apexrest/{ns}/v2/cpq/carts/{workingCartId}/items
        # with a list of { "itemId": <PricebookEntryId> }. This is the
        # documented Vlocity CPQ REST pattern — the UI tunnels the same
        # call through Aura (CpqAppHandler.postCartsItems). Items land on
        # the WC; Step 15 copies them onto the EQ.
        #
        # NOTE on DIA's required attribute (``ATTR_BANDWIDTH``):
        # DIA has ATTR_BANDWIDTH configured as **required with a null
        # default**. The Vlocity v2 POST endpoint validates required
        # attributes and emits:
        #   [ERROR] Required attribute missing for Dedicated Internet Access (DIA).
        ...
        # record snapshot and PUTs it back — a flow we replicate in
        # Step 14 via ``cpq_configure_line_item_attributes``.
        #
        # The UI also produces a QuoteLineItem at POST time even when
        # the required-attr validation fails. We tolerate those two
        # specific validation messages here, SOQL-verify the QLI was
        # created, and Step 14 supplies the missing values via the
        # GET-modify-PUT snapshot flow — satisfying the validation.
        t.start_step(13, "Add DIA + Router to Working Cart via CPQ v2 REST")
        try:
            add_resp = sf_api.cpq_post_cart_items(
                self.working_cart_id,
                [
                    {"itemId": self.dia_pbe_id},
                    {"itemId": self.router_pbe_id},
                ],
                name=f"CPQ v2: POST /carts/{self.working_cart_id}/items (DIA + Router)",
                # DIA's required-attribute validation at POST time is a
                # known Vlocity quirk — the QLI is still created, so we
                # tolerate the specific messages here and satisfy them
                # in Step 14's snapshot PUT. Any other ERROR still hard-fails.
                tolerate_messages=[
            ...
            tolerated = (
                add_resp.get("__tolerated_errors")
            ...
            if tolerated:
                t.add_assertion(
                    f"Tolerated DIA required-attr validation: {'; '.join(tolerated)[:300]}",
                    True,
                )

            # Capture the WC's QuoteLineItem IDs for Step 14's attribute
            # PUT. Shape of the response varies by CPQ version — look up
            # via SOQL (filtering by the WC's Id) as the reliable source.
            # ProductCode lives on Product2, not QuoteLineItem — traverse
            # the relationship (Product2.ProductCode) to retrieve it.
            ...
            for row in qli_rows:
                ...
                if pc == DIA_PRODUCT_CODE:
                    self.dia_line_item_id = row["Id"]
                elif pc == ROUTER_PRODUCT_CODE:
                    self.router_line_item_id = row["Id"]
            t.add_assertion(
                f"QuoteLineItems on WC after add: DIA={self.dia_line_item_id}, "
                f"Router={self.router_line_item_id} (total rows={len(qli_rows)})",
                bool(self.dia_line_item_id or self.router_line_item_id),
            )
            if not (self.dia_line_item_id or self.router_line_item_id):
                raise RuntimeError(
                    "No QuoteLineItems found on Working Cart after postCartsItems. "
                    "Tolerated validation errors are only safe when the QLI is "
                    "still created — this run indicates the records were NOT "
                    "persisted. "
...
# Ashish commented as it was failing
        # # ── Step 14: Configure attributes (DIA bandwidth + Router provider)
        ...
        # #
        # # We target the WORKING CART here — attribute values travel with
        # # the items when AddQMQGToWC_CopyToEQ copies them onto the EQ.
        # t.start_step(
        #     14,
        #     f"Configure attributes on Working-Cart items — DIA bandwidth "
        #     f"'{DIA_BANDWIDTH}', Router '{ROUTER_PROVIDED_BY}'",
        # )
        # try:
        #     updates: list[dict] = []
        #     if self.dia_line_item_id:
        #         updates.append(
        #             {
        #                 "itemId": self.dia_line_item_id,
        #                 "attributeValues": {DIA_BANDWIDTH_ATTR: DIA_BANDWIDTH},
        #             }
        #         )
        #     if self.router_line_item_id:
        #         updates.append(
        #             {
        #                 "itemId": self.router_line_item_id,
        #                 "attributeValues": {ROUTER_PROVIDED_BY_ATTR: ROUTER_PROVIDED_BY},
        #             }
        #         )
        #     if not updates:
        #         raise RuntimeError("No line items to update — Step 13 didn't capture any.")
        ...
        # # The IP ``AddQMQGToWC_CopyToEQ`` copies WC line items onto the
        # # EQ, attaches them to the supplied QuoteMember Id(s), then
        # # disposes the Working Cart. WorkingCartId != SalesQuoteId is
        # # critical — when they are the same, the IP deletes the EQ
        # # itself (see the Phase-3 header comment for the longer story).
        # #
        # # Payload shape (captured from TC1):
        # #   {"WorkingCartId": "<WC_Id>",
        # #    "MemberIds": [{"Id": "<QuoteMember_Id>"}],
        # #    "QMBData": {},
        # #    "QuoteGroupIds": [],
        # #    "SalesQuoteId": "<EQ_Id>",
        ...
        # t.start_step(
        #     15,
        #     f"Copy Working-Cart items onto Enterprise Quote (and link to "
        ...
        # # After CopyToEQ:
        # #   - The EQ (self.quote_id) MUST still exist — tripwire against
        # #     a future regression where WC==EQ slips back in.
        # #   - The EQ's QuoteLineItems must reference the QuoteMember
        # #     created in Step 10. Field name varies by namespace; we
        # #     resolve it via describe (pick_field) so the test works on
        # #     either vlocity_cmt- or omnistudio-flavored orgs.
        # t.start_step(
        #     16,
        #     "Verify Enterprise Quote survived + QuoteLineItems reference "
        ...
        #     )
        #     t.add_assertion(
        #         f"DIA line item on EQ (code={DIA_PRODUCT_CODE})",
        #         DIA_PRODUCT_CODE in product_codes,
        #     )
        #     t.add_assertion(
        #         f"Router line item on EQ (code={ROUTER_PRODUCT_CODE})",
        #         ROUTER_PRODUCT_CODE in product_codes,
        #     )

        #     # 16c — Verify attributes actually landed on the EQ line items.
        #     # Step 14 PUTs attribute values onto the Working Cart items;
        ...
        #         # Shape 3: nested by category — {"group": [{"code":..., "value":...}]}
        ...
        #     if attr_field:
        #         dia_row = next(
        #             (r for r in rows if ((r.get("Product2") or {}).get("ProductCode")) == DIA_PRODUCT_CODE),
        #             None,
        #         )
        #         router_row = next(
        #             (r for r in rows if ((r.get("Product2") or {}).get("ProductCode")) == ROUTER_PRODUCT_CODE),
        #             None,
        #         )
        #         if dia_row:
        #             dia_bw = _attr_value(dia_row.get(attr_field), DIA_BANDWIDTH_ATTR)
        #             t.add_assertion(
        #                 f"DIA {DIA_BANDWIDTH_ATTR} on EQ = {dia_bw!r} "
        #                 f"(expected {DIA_BANDWIDTH!r})",
        #                 dia_bw == DIA_BANDWIDTH,
        #             )
        #         else:
        #             t.add_assertion(
        #                 f"DIA row not present on EQ — cannot verify {DIA_BANDWIDTH_ATTR}",
        #                 False,
        #             )
        #         if router_row:
        #             router_pb = _attr_value(
        #                 router_row.get(attr_field), ROUTER_PROVIDED_BY_ATTR
        #             )
        #             t.add_assertion(
        #                 f"Router {ROUTER_PROVIDED_BY_ATTR} on EQ = {router_pb!r} "
        #                 f"(expected {ROUTER_PROVIDED_BY!r})",
        #                 router_pb == ROUTER_PROVIDED_BY,
        #             )
        #         else:
        #             t.add_assertion(
        #                 f"Router row not present on EQ — cannot verify "
        #                 f"{ROUTER_PROVIDED_BY_ATTR}",
        #                 False,
        #             )
        #     else:
        #         t.add_assertion(
