...
# sfauto — full reference

> Deep reference. For install and first run, start at the [README](../README.md).
...
**[docs/CI.md](CI.md) is the full setup guide.** The short version:
...
| `SF_JWT_KEY_FILE`, `SF_JWT_USERNAME` | — | JWT bearer — the only headless flow that yields a `web`-scoped token, so it is what makes UI login work on SSO/MFA orgs. See [docs/AUTHENTICATION.md](AUTHENTICATION.md) |
